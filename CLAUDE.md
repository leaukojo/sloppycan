...
- **Flavored contract signals are packed by their flavor module, not by `CAN_MAP`:** the contract's
  ...
  and `IN_SOURCES` warns). **A third registry, `window.carlitoUplinkOverrides`, is checked before
