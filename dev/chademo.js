...
//   sloppycan.js  startup: window.ingestFrame = ingestFrame;   (used by the demo loop)
//   index.html    view-tabs button + #chademoWrap content + <script src="chademo.js" defer>
//   sloppycan.js  switchViewTab(): chademo tab toggle + wrap show/hide + demoMaybeSwitch
//   sloppycan.js  demo base-traffic engine calls window.chademoDemoLoopStart/Stop
//
// ── Inject CSS ────────────────────────────────────────────────────────────────
(function () {
...
.chademo-note a { color:var(--blue); text-decoration:none; white-space:nowrap; margin-left:auto; }
.chademo-tbl a.cm-seecan { color:var(--blue); text-decoration:none; }
.chademo-state {
...
.chademo-pause-btn {
  margin-left:10px; vertical-align:middle; font-size:11px; padding:3px 11px;
...
.chademo-pause-btn:hover { background:var(--bg3); color:var(--text); }
...
const CHADEMO_LOG_MAX = 500;   // passed to the factory as cfg.logMax
...
const cmRelTs = window.canRelTs;        // shared formatter (defined in sloppycan.js)
...
function chademoIngestFrame(frame) {
  ...
  chademoTab.pushLog({ id: frame.id, data, ts, val: chademoValueStr(frame.id, data) });
}

function chademoClear() {
  // NB: do NOT stop the demo loop here - the demo timer is owned by the global demo lifecycle
  // (chademoDemoLoopStart/Stop, driven by the base-traffic engine). Clearing only wipes captured
  // traffic; the Pause/Resume-demo control lives in the session panel and must survive a Clear.
...
// ── Render ────────────────────────────────────────────────────────────────────
// Session and Frame Log are stacked in one view (Session on top via renderTop, Log below) -
// both render every tick through the shared factory, like the XCP / CANopen tabs.
...
function chademoRenderSession() {
  ...
  if (!CM.lastIds.size) {
    ...
    el.innerHTML = '<div class="chademo-empty">No CHAdeMO frames received yet.<br>' +
      ...
      'In <b>Demo</b> mode, open this tab to replay a looping charging session.</div>';
    return;
  }
  // Build the header (state pill + pause button) and dashboard container ONCE so the
  // button is a stable DOM node - rebuilding innerHTML every tick (~10×/s during the
  // demo) would tear the button down and make it impossible to click.
  if (el.dataset.built !== '1') {
    el.innerHTML =
      '<div style="margin-bottom:4px">' +
      '<span id="chademo-state-pill" class="chademo-state"></span>' +
      '<button id="chademo-pause-btn" class="chademo-pause-btn" style="display:none" onclick="chademoDemoPauseToggle()"></button>' +
      '</div><div id="chademo-dash"></div>';
    el.dataset.built = '1';
  }
  const st = chademoSessionState();
  const pill = document.getElementById('chademo-state-pill');
  pill.className = 'chademo-state' + (st.kind ? ' ' + st.kind : '');
  pill.textContent = st.label;
  const btn = document.getElementById('chademo-pause-btn');
  if (chademoDemoTimer) {
    btn.style.display = '';
    const label = chademoDemoPaused ? '▶ Resume demo' : '❚❚ Pause demo';
  ...
  document.getElementById('chademo-dash').innerHTML = `
...
// ── Demo session (base traffic) - loops a handshake + charging ramp ────────────
// Driven by sloppycan.js's demo base-traffic engine when CHAdeMO is the active
// base traffic. Replays capability exchange, then a charging ramp that restarts
// on completion so the session repeats indefinitely.
...
function chademoDemoCapability() {
...
let chademoDemoSoc = 20, chademoDemoT = 0, chademoDemoRestartAt = 0, chademoDemoPaused = false;
function chademoDemoLoopStart() {
  if (chademoDemoTimer) return; // already running
  chademoDemoSoc = 20; chademoDemoT = 0; chademoDemoRestartAt = 0; chademoDemoPaused = false;
  chademoDemoCapability();
  chademoDemoTimer = setInterval(() => {
    if (chademoDemoPaused) return;                 // "Pause demo" button
    if (chademoDemoRestartAt) {                     // idle 5 s after completion, then restart
      if (Date.now() < chademoDemoRestartAt) return;
      chademoDemoRestartAt = 0; chademoDemoSoc = 20; chademoDemoT = 0; chademoDemoCapability();
    }
    chademoDemoT++;
    chademoDemoSoc = Math.min(80, chademoDemoSoc + 1);
    const soc = chademoDemoSoc;
    ...
    const last = (soc >= 80) || (chademoDemoT >= 60); // ~6 s per cycle
    ...
    if (last) chademoDemoRestartAt = Date.now() + 5000; // wait 5 s, then loop
  }, 100);
}
function chademoDemoLoopStop() {
  if (chademoDemoTimer) { clearInterval(chademoDemoTimer); chademoDemoTimer = null; }
  chademoDemoPaused = false; chademoDemoRestartAt = 0;
}
function chademoDemoPauseToggle() {
  chademoDemoPaused = !chademoDemoPaused;
  chademoTab.markDirty(); chademoTab.render();
}

// ── Stacked log tab (render loop + frame-log table via the shared factory) ─────
// Passive monitor: render-loop + log table only (no TX, so no ready/hint/readout helpers).
const chademoTab = window.makeStackedLogTab({
  wrapId: 'chademoWrap', logElId: 'chademo-log',
  tableClass: 'chademo-tbl', logMax: CHADEMO_LOG_MAX,
  theadHtml: '<tr><th>Time</th><th>ID</th><th>Message</th><th>Bytes</th><th>Decoded</th></tr>',
  emptyHtml: '<div class="chademo-empty">No CHAdeMO frames in log yet.</div>',
  renderTop: chademoRenderSession,
  rowHtml: (e) => {
    const m = CHADEMO_MSGS[e.id];
    const raw = Array.from(e.data).map(b => b.toString(16).toUpperCase().padStart(2, '0')).join(' ');
    return `<tr>
        <td class="cm-ts"><a href="#" onclick="isotpSeeCanTraffic(${e.ts});return false" title="See this frame in the Traffic Dump" class="cm-seecan">⊞</a> ${cmRelTs(e.ts)}</td>
...
window.chademoDemoLoopStart = chademoDemoLoopStart;
window.chademoDemoLoopStop = chademoDemoLoopStop;
window.chademoDemoPauseToggle = chademoDemoPauseToggle;
