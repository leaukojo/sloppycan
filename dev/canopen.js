...
// rendering live here. Same bolt-on pattern as xcp.js / chademo.js.
//
...
.co-logbar a { color:var(--blue); text-decoration:none; white-space:nowrap; font-weight:400; font-size:11px; margin-left:auto; }
.co-tx-hint { font-size:11px; color:var(--amber); padding:2px 0 0 0; font-family:var(--sans); }
.co-tbl a.co-seecan { color:var(--blue); text-decoration:none; }
.co-config {
...
const coRelTs    = window.canRelTs;        // shared formatters (defined in sloppycan.js)
const coHexBytes = window.canHexBytes;
const coParseInt = window.canParseIntAuto;
function coParseId(str) { const v = parseInt((str || '').replace(/^0x/i, ''), 16); return Number.isFinite(v) ? v : null; }
function coU32(d, i) { return (d[i]||0) + (d[i+1]||0) * 0x100 + (d[i+2]||0) * 0x10000 + (d[i+3]||0) * 0x1000000; } // LE u32 (no <<24 sign trap; missing bytes on a short frame → 0, not NaN)

// Live validation for the form fields: toggle .invalid (red) as the user types. The real
// parse/clamp still happens at click time in the canopenSdo*/canopenNmtBtn handlers (backstop).
function coFlag(el, ok) { if (el) el.classList.toggle('invalid', !ok); }
function coValNode(el)    { const v = parseInt(el.value, 10); coFlag(el, Number.isFinite(v) && v >= 0 && v <= 127); }   // config default node
function coValTimeout(el) { const v = parseInt(el.value, 10); coFlag(el, Number.isFinite(v) && v > 0); }
function coValSdoNode(el) { const v = parseInt(el.value, 10); coFlag(el, Number.isFinite(v) && v >= 1 && v <= 127); }
function coValIndex(el)   { const v = coParseInt(el.value); coFlag(el, Number.isFinite(v) && v <= 0xFFFF); }
function coValSub(el)     { const v = coParseInt(el.value); coFlag(el, Number.isFinite(v) && v <= 0xFF); }
function coValValue(el)   { coFlag(el, Number.isFinite(coParseInt(el.value))); }
function coValNmt(el)     { const t = el.value.trim(); if (t === '' || /^all$/i.test(t)) { coFlag(el, true); return; }
                           const v = parseInt(t, 10); coFlag(el, Number.isFinite(v) && v >= 0 && v <= 127); }
...
// ── Per-type decoders (PURE: return a summary string, no coNodes side effects) ────────────────
...
function coDecodeHb(d) {
  const st = d[0] & 0x7F;
  return CO_NMT_STATE[st] || ('state ' + coH(st));
}

// Optimistically reflect a commanded NMT state into the node map. UNCONFIRMED: there's no ack on the
// wire, so a real node may not actually obey - this is a UI guess (applied for both observed bus
// commands and our own self-sent ones). Kept separate from the pure decode above to make that clear.
function coApplyNmtState(d) {
...
// A heartbeat is the node reporting its OWN state - reflect it as observed truth (not a guess).
function coApplyHbState(node, d) {
  coNodeFor(node).state = d[0] & 0x7F;
}
...
function coExpVal(d, size) {
  ...
  let v = 0; for (let i = 0; i < size; i++) v += (d[4 + i] || 0) * 2 ** (8 * i); // LE unsigned (avoid <<24 sign trap; coerce missing bytes on a short frame to 0)
...
function canopenIngestFrame(frame) {
  ...
  switch (c.kind) {
    case 'nmt':    summary = coDecodeNmt(d); coApplyNmtState(d); break;
    case 'hb':     summary = coDecodeHb(d); coApplyHbState(c.node, d); break;
  ...
  coTab.pushLog({ ts, id, node: c.node, kind: c.kind, type: c.type, data: Array.from(d), summary, emcy });
}
...
// ── Active client ─────────────────────────────────────────────────────────────
function coReady() { return coTab.ready(); }
function coReadout(msg, isErr) { coTab.readout(msg, isErr); }
// Reveal the TX-disabled hint - called when a send is attempted while the bus isn't ready.
function coShowTxHint() { coTab.showTxHint(); }
...
async function coTx(id, dlc, bytes, dir, dec) {
  ...
  if (!ok) return false;
  coTab.pushLog({ ts: Date.now(), id, node: id & 0x7F, kind: dir === 'rx' ? 'sdo_rx' : 'nmt', type: coClassify(id).type, data: bytes.slice(), summary: dec });
...
async function coSdoRead(node, index, sub) {
  if (!coReady()) { coShowTxHint(); return; }
...
// Hard cap on segmented-upload reassembly. A server (or line noise) that keeps dribbling
// non-final segments re-arms the watchdog each one, so without this the buffer grows forever.
const CO_SDO_SEG_MAX = 65536;
...
function coSdoResolve(id, dec, d) {
  ...
  if (dec.kind === 'upseg') {                          // a segment of data
    ...
    clearTimeout(coPending.timer);
    if (coPending.buf.length > CO_SDO_SEG_MAX) { coSdoFail(`segmented upload exceeded ${CO_SDO_SEG_MAX} bytes - aborting`); return; }
    coPending.timer = coArmTimer(coPending.node);
    if (last) {
...
async function coNmt(cmd, target) {
  ...
  // Optimistically reflect the commanded state for live UI feedback (unconfirmed - see coApplyNmtState).
  coApplyNmtState([cmd, target]);
  const ok = await window.fuzzTxFrame(0x000, false, 2, [cmd, target]);
  coTab.pushLog({ ts: Date.now(), id: 0x000, node: 0, kind: 'nmt', type: 'NMT', data: [cmd, target], summary: label });
  if (window.canopenDemoActive && window.canopenDemoActive()) coDemoApplyNmt(cmd, target);
  coReadout(ok ? ('NMT: ' + label) : ('NMT TX failed: ' + label));
  coTab.markDirty();
}
...
async function coSync() {
  ...
  coTab.pushLog({ ts: Date.now(), id: 0x080, node: 0, kind: 'sync', type: 'SYNC', data: [], summary: 'SYNC' });
  coReadout('SYNC sent'); coTab.markDirty();
}
...
// ── Render ────────────────────────────────────────────────────────────────────
// Nodes & Control and Message Log are stacked in one view (Nodes on top, Log below) -
// both render every tick, like the CHAdeMO / XCP tabs.
function coUpdateButtons() {
  // Buttons are gated only by an in-flight transaction; bus-readiness is surfaced on attempt
  // via coShowTxHint() (so the hint isn't shown before the user tries to send anything).
  document.querySelectorAll('#canopenWrap .co-tx-btn').forEach(b => { b.disabled = !!coPending; });
  if (coReady()) { const hint = document.getElementById('co-tx-hint'); if (hint) hint.style.display = 'none'; }
}
...
function coRenderNodes() {
  ...
  if (!coNodes.size) {
    el.innerHTML = '<div class="co-empty">No nodes seen yet.<br>' +
      ...
      'In <b>Demo</b> mode, switch the demo base traffic to CANopen (open this tab) for simulated nodes - no hardware needed.</div>';
    return;
  }
...
// Nodes & Control and Message Log are stacked in one view (Nodes on top via renderTop, Log below).
const CO_TYPE_CLS = { emcy: 'emcy', sdo_rx: 'sdo', sdo_tx: 'sdo', hb: 'hb', nmt: 'nmt', pdo: 'pdo' };
const coTab = window.makeStackedLogTab({
  wrapId: 'canopenWrap', logElId: 'co-log',
  readoutElId: 'co-readout', txHintElId: 'co-tx-hint',
  tableClass: 'co-tbl',
  theadHtml: '<tr><th>Time</th><th>COB-ID</th><th>Node</th><th>Type</th><th>Bytes</th><th>Decoded</th></tr>',
  emptyHtml: '<div class="co-empty">No CANopen frames yet.<br>The COB-ID (11-bit) is split into a function code + node-ID to classify each frame.</div>',
  onTick: coUpdateButtons,
  renderTop: coRenderNodes,
  coalesce: (prev, e) => e.kind === 'pdo' && prev.kind === 'pdo' && prev.id === e.id && (e.ts - prev.ts) < CO_PDO_COALESCE_MS,
  rowHtml: (e) => {
    const cnt = e.count && e.count > 1 ? ` <span style="color:var(--text3)">×${e.count}</span>` : '';
    return `<tr class="${e.emcy ? 'emcy-row' : ''}">
        <td class="co-ts"><a href="#" onclick="isotpSeeCanTraffic(${e.ts});return false" title="See this frame in the Traffic Dump" class="co-seecan">⊞</a> ${coRelTs(e.ts)}</td>
        <td class="co-id">${coH(e.id, 3)}</td>
        <td class="co-node">${e.node || '-'}</td>
        <td class="co-type ${CO_TYPE_CLS[e.kind] || ''}">${e.type}</td>
...
window.canopenDemoStart = coStartDemo; // demo base-traffic engine starts/stops the node
window.canopenDemoStop = coStopDemo;
