...
- **The two sides are linked both ways, and the mechanisms are not symmetric.** *Game → panel* is free: the telemetry already says which machine is on the link, so the windows follow it silently. *Panel → game* costs a **reload**, because the vehicle is a **boot parameter** - the game's `?level=&vehicle=` deep link, parsed by its `BootParams`, taking a VARIANT id (`drone`, `sedan`) rather than a family. `carlito.js` exposes `window.carlitoSelectVehicle(variant)` for it, and `switchViewTab('dronecan')` calls `window.vehiclePanelRequestForProto('dronecan')` - the same entry every other protocol view uses - which **asks first** (`demoMaybeSwitch`'s precedent - a tab click that restarts someone's drive unannounced is a worse surprise than a dialog) and stays quiet when there is nothing to change. The request is **spent on the load it causes**: a later Reload or channel switch goes back to the game's own saved session rather than pinning it. **There is deliberately no "change vehicle" contract signal** - the contract carries a vehicle's CAN signals, not commands to the shell around it - which is also why this needed no game-side change and works against the already-deployed build.
- **Vehicle detection.** The game publishes `signals_for_vehicle()`, so a **drone-only `dir:'out'` signal being present IS the vehicle** - there is no `vehicle_type` on the wire and there does not need to be. The test is `vehicle-panel.js`'s (`vehiclePanelIsFamily('drone', t)`), off the contract's `vehicles` field, so a new drone signal needs no edit here. `carlito.js` pushes each payload to `carlitoOnTelemetry`, which `vehicle-panel.js` owns and fans out, with `null` when the link goes away. Only an **automatic** open is undone by the automatic close: a window closed by hand is not re-opened on the next frame.
- **It replaces the RAMN pair rather than joining it.** `vehiclePanelClaim('drone', open, 'pair')` - closes the RAMN dashboard **and** Control, and puts back whatever was up when it took over (remembered in `vehicle-panel.js`). A car's pedals, gearbox and cluster describe nothing about a quadcopter, and while the panel is flying it has taken accel/brake/steer off those controls anyway - so leaving them up is two control surfaces for one aircraft, one of which does nothing. A RAMN pair the user had already closed stays closed.
...
**Integration points:** `index.html`: `#droneBtn` (`onclick="droneToggle()"`) + `<script src="drone.js" defer>` **after `ramn.js` + `dronecan.js`, before `carlito.js`** (which snapshots the override registry at eval) + an onboarding chip. `sloppycan.js`: `'droneBtn'` in `_buttonsWrap`'s id list, and `switchViewTab` → `if (name === 'dronecan' && window.vehiclePanelRequestForProto) window.vehiclePanelRequestForProto('dronecan');`. `vehicle-panel.js`: `vehiclePanelSubscribe` / `vehiclePanelIsFamily` / `vehiclePanelRegister` (with `btn: 'droneBtn'`, so the button's visibility is central too) / `vehiclePanelClaim` / `vehiclePanelRequestForProto`. `carlito.js`: `window.carlitoUplinkOverrides`. `ramn.js`: `makeFloating`'s `closeSel` option. `dronecan.js`: `arm`/`climb` on `dcCtl` and the uplink registry, plus the accessor exports. All window DOM + CSS injected by `drone.js`. Not loaded on `carlito-bridge.html`, which has no dashboards.
...
## Vehicle panels: the shared mechanism (`vehicle-panel.js`)

The parts of `drone.js` that are not about drones, so `truck.js` / `tractor.js` / `boat.js` are each only their own instrument panel. Loads after `carlito_contract.js` (read at eval), before every panel.

1. **Telemetry fan-out.** `carlito.js` calls exactly one `window.carlitoOnTelemetry`, so panels `vehiclePanelSubscribe(fn)` rather than assign it - otherwise the last script loaded is silently the only listener. A subscriber that throws is warned about once and skipped.
2. **Which machine is on the link.** `vehiclePanelIsFamily(family, t)`: a `dir:'out'` signal the contract gives to **exactly one** family being present IS the vehicle. A name shared with the car says nothing about who published it. Cached per family.
3. **The RAMN handoff, one panel at a time.** `vehiclePanelClaim(id, open, takes)`; `takes` is what the panel REPLACES - `'pair'` (dashboard + Control, for a machine the RAMN controls do not fit) or `'dash'` (`ramnSetDashOpen`, the cluster alone, for one they do). Claiming while another panel holds the screen calls its `close()`: a **handover**, which a vehicle change really is, because `carlito.js` reloads the iframe for `?vehicle=` **without nulling its last payload** - the switch never passes through "no link". So the RAMN record belongs to the **chain**: re-reading `ramnIsOpen()` on a handover records "the cluster was down" (it is - the previous panel put it down) and the last panel out never restores it. `ramnPairTaken` is the chain's **maximum** for the same reason, while the *closing* uses the claim's own `takes` so a road vehicle never shuts the Control Panel it is driven with.
4. **The deep link back.** `vehiclePanelRequestVehicle(id, why)` - one confirm for every panel, because asking the game for a vehicle costs a **reload** (`?vehicle=`, a boot param). Quiet when there is nothing to change. `vehiclePanelRequestForProto(mode)` is the one table - `dronecan` → drone, `j1939` → truck, `iso11783` → tractor, `nmea2000` → boat - under one rule: **a protocol view asks for its machine when the user selects that protocol.** The DroneCAN tab holds one protocol, so showing it is the selection; the J1939 tab holds three, so its mode button is - a tab opened to read frames must not offer to restart a drive.
5. **Toolbar buttons.** A panel's button and onboarding chip are up only while its machine is on the link **or** its window is open, so the header carries at most one. All four ship `style="display:none"`; a shown/hidden button changes the header width, so `reflowHeader()` re-runs on each change (and `syncButtons` writes nothing when the visible set is unchanged - it is on a 20 Hz path).

`vehiclePanelRegister({ id, family, variant, label, verb, btn, close, isOpen })`: `variant` is the **VehicleCatalog variant** id (`semi`, `tractor-kenney`, `boat-speed-a`, `drone`), not the family; `verb` is how the machine is operated (a drone is *flown*, default *Drive*); `isOpen()` keeps the button up while the window it closes is open.

**Integration:** `index.html` - script after `carlito_contract.js`, `style="display:none"` on all four panel buttons. `ramn.js` - `ramnSetDashOpen`. `j1939.js` - `j1939ProtoClick()` → `vehiclePanelRequestForProto(mode)`, **guarded on `j1939ProtoMode === mode`** (in demo `demoMaybeSwitch` is two confirms of its own and can be declined). `sloppycan.js` - `switchViewTab('dronecan')` → `vehiclePanelRequestForProto('dronecan')`, and `reflowHeader`. `carlito.js` - owns `carlitoOnTelemetry`, calls `carlitoSelectVehicle`.

**Revert:** delete the file and put `drone.js` back on its own `carlitoOnTelemetry` assignment + local `handoffRamn`; delete the panels below.

---

## Truck / Tractor / Boat dashboards (`truck.js`, `tractor.js`, `boat.js`)

One floating window each in `ramn.js`'s shape (`makeFloating`, own `closeSel`), following the link through `vehicle-panel.js`. Buttons `#truckBtn` / `#tractorBtn` / `#boatBtn` after `#droneBtn`.

- **Three files, not one parameterised one.** Currently the RAMN cluster verbatim, off the same `window.ramnGetState()` - a **starting point**. Each is meant to be edited apart onto its own signals (`air_primary` / `retarder_state` / `trailer_*`; `hitch_pos_actual` / `pto_rpm` / `draft_force`; `rudder_actual` / `trim`), which arrive in the payload the file is already handed. A shared builder would have to be un-shared on the first such edit.
- **They take `'dash'`, not `'pair'`** - all three are driven with the RAMN controls, so the Control Panel keeps driving. The one structural difference from the drone pair.
- **They claim no uplink signal.** Nothing writes `carlitoUplinkOverrides` / `carlitoUplinkSources`.
- **Render** is rAF-throttled off the telemetry push (~20 Hz), skipped while closed; with no vehicle of that family on the link the cluster shows its resting state, not the last one's readings.
- **No contract change** - detection reads signals already in v33, so not a paired promote.

**Integration:** `index.html` - three `<script defer>` tags after `drone.js`, three buttons, three onboarding chips. `sloppycan.js` - the three ids in `_buttonsWrap`. Everything else through `window.vehiclePanel*`. Not loaded on `carlito-bridge.html`.

**Revert:** delete the file, its `<script>` tag, button, onboarding chip and `_buttonsWrap` id.
