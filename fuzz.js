// ── fuzz.js ───────────────────────────────────────────────────────────────────
// Self-contained CAN fuzzing module (same bolt-on pattern as graph.js /
...
// Integration points in the core (all optional-chained):
...
let fuzzCount = 0;
let fuzzBusy = false;
const FUZZ_MAX_FAILS = 5;   // auto-stop after this many consecutive TX failures (e.g. adapter unplugged)
let fzFailStreak = 0;
...
function fuzzCfgChange() {
  ...
  fuzzCfg.dlcFixed = Math.min(8, Math.max(0, parseInt(document.getElementById('fuzzDlcFixed').value, 10) || 0));
  ...
  if (fuzzCfg.payMode === 'bitgrid') {
    ...
    if (pos.length) {
      if (fuzzCfg.bitPat === 'sweep') {
        ...
        const n = fzSeqBits++;
        // Use float division, not `>>`, to address bit k: `n >> k` masks k to 0–31 so a fuzz grid with
        // >31 cells would alias/lose high bits. This keeps a clean sweep up to ~52 cells (Number safe int).
        pos.forEach(([r, c], k) => { if (Math.floor(n / 2 ** k) % 2) out[r] |= (1 << (7 - c)); });
      } else { // rand
...
async function fuzzTick() {
  if (fuzzBusy) return;
  // Gate on full TX-readiness (open bus AND not listen-only), not just bus-ready, so toggling
  // listen-only mid-run halts immediately instead of waiting for FUZZ_MAX_FAILS write failures.
  if (!fuzzTxReady()) { fuzzStop(); return; }
  fuzzBusy = true;
  try {
    for (let b = 0; b < fuzzCfg.burst; b++) {
      ...
      if (ok) { fuzzCount++; fzFailStreak = 0; }
      else if (++fzFailStreak >= FUZZ_MAX_FAILS) {
        if (window.log) window.log(`Fuzzer stopped: ${FUZZ_MAX_FAILS} consecutive TX failures`, 'err');
        fuzzStop();
        break;
      }
    }
...
function fuzzToggleRun() {
  ...
  // TX disabled when no open bus or listen-only - reveal the inline hint (instead of an alert).
  if (!fuzzTxReady()) { const hint = document.getElementById('fuzzTxHint'); if (hint) hint.style.display = ''; return; }
  ...
  fuzzCount = 0;
  fzIdCur = fzObsCur = fzDlcCur = fzSeqBits = fzSweepCur = fzFailStreak = 0;
...
// TX is allowed only on an open bus that isn't listen-only (mirror obdBusReady).
function fuzzTxReady() {
  if (window.txReady) return window.txReady();
  return !!(window.fuzzBusReady && window.fuzzBusReady()) && !document.getElementById('listenOnly').checked;
}
...
function fuzzUpdateIndicator() {
  ...
  // Only ever hide the hint here (when ready or running); it's revealed on attempt in fuzzToggleRun,
  // so it isn't shown before the user tries to start fuzzing.
  const hint = document.getElementById('fuzzTxHint');
  if (hint && (fuzzRunning || fuzzTxReady())) hint.style.display = 'none';
...
window.fuzzOnShow = function () {
  ...
  fuzzUpdateIndicator();   // refresh the TX-disabled hint for the current bus state
};
